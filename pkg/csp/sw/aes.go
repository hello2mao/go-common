...
func (e *aescbcpkcs7Encryptor) Encrypt(k csp.Key, plaintext []byte, opts csp.EncrypterOpts) ([]byte, error) {
	switch o := opts.(type) {
	case *csp.AESCBCPKCS7ModeOpts:
...
func (*aescbcpkcs7Decryptor) Decrypt(k csp.Key, ciphertext []byte, opts csp.DecrypterOpts) ([]byte, error) {
	// check for mode
	switch opts.(type) {
	case *csp.AESCBCPKCS7ModeOpts, csp.AESCBCPKCS7ModeOpts:
		// AES in CBC mode with PKCS7 padding
