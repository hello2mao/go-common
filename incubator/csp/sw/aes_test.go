...
import (
	...
	"github.com/hello2mao/go-common/incubator/csp/mocks"
...
// TestAESRelatedUtilFunctions tests various functions
...
func TestAESCBCPKCS7EncryptorDecrypt(t *testing.T) {
	...
	_, err = encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{IV: []byte{1}})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "Invalid IV. It must have length the block size")

	_, err = encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{IV: []byte{1}, PRNG: rand.Reader})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "Invalid options. Either IV or PRNG should be different from nil, or both nil.")

	_, err = encryptor.Encrypt(k, msg, csp.AESCBCPKCS7ModeOpts{})
	assert.NoError(t, err)

	ct, err := encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{})
	assert.NoError(t, err)
	...
	msg2, err := decryptor.Decrypt(k, ct, &csp.AESCBCPKCS7ModeOpts{})
	assert.NoError(t, err)
...
func TestAESCBCPKCS7EncryptorWithIVSameCiphertext(t *testing.T) {
	...
	ct, err := encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{IV: iv})
	assert.NoError(t, err)
	assert.NotNil(t, ct)
	assert.Equal(t, iv, ct[:aes.BlockSize])

	ct2, err := encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{IV: iv})
	assert.NoError(t, err)
	assert.NotNil(t, ct2)
...
func TestAESCBCPKCS7EncryptorWithRandSameCiphertext(t *testing.T) {
	...
	r = mrand.New(mrand.NewSource(0))
	ct, err := encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{PRNG: r})
	assert.NoError(t, err)
	assert.NotNil(t, ct)
	assert.Equal(t, iv, ct[:aes.BlockSize])

	r = mrand.New(mrand.NewSource(0))
	ct2, err := encryptor.Encrypt(k, msg, &csp.AESCBCPKCS7ModeOpts{PRNG: r})
	assert.NoError(t, err)
	assert.NotNil(t, ct2)
	assert.Equal(t, iv, ct2[:aes.BlockSize])
